...
config = LoraConfig(
    ...
    # modules_to_save=['word_embeddings'],  # 也可以对basemodel中的参数进行训练
)
...
# 多个 LoRA 保存到同一个目录（多 adapter）
model.save_pretrained("./adapters/lora_task_A")
# model.save_pretrained("./adapters/lora_task_B")
# model.save_pretrained("./adapters/lora_task_C")

# 对adapter进行加载，可以加载多个适配器
modelA = PeftModel.from_pretrained(model, "./adapters/lora_task_A", "A")
# modelB = PeftModel.from_pretrained(model, "./adapters/lora_task_B", "B")

# 面对具体的任务设置合适的适配器
modelA.set_adapter("A")
print("已切换到 LoRA Adapter A！")

# 可以查看当前激活的适配器
print(modelA.active_adapter)

import warnings
warnings.filterwarnings("ignore") # 减少警告信息输出

# 合并适配器到基础模型,就是权重相加
# 在未merge之前，模型类型是peftmodel，所以保存保存的是lora部分，合并之后，模型类型是AutoModel，保存的是完整的融合模型
merge_model= modelA.merge_and_unload() 
