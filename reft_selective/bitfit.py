...
# selective本身就是对模型中的部分参数进行调参，而 Bitfit 算法就是对模型中的bias偏置项进行调整
# Bitfit
# 统计bias参数数量
num_param = 0
for name, param in model.named_parameters():
    if "bias" not in name:
        ...
        # param.requires_grad = True，这一行可以省略，参数默认是True
        num_param += param.numel()

print(f"bias参数数量: {num_param}")

# bias参数对总参数量的占比
print(f"bias参数对总参数量的占比: {num_param / sum(param.numel() for param in model.parameters())}")
